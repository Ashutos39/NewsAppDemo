//
//  NewsApiDemoCoreDataStack.swift
//
//  NewsApiDemoDemoApp
//
...
final class NewsApiDemoCoreDataStack: NewsApiDemoCoreDataStackProtocol {
    
    static let shared = NewsApiDemoCoreDataStack()
    
    private(set)var persistentContainer: NSPersistentContainer
    private(set) var config: NewsApiDemoCoreDataStackConfiguration
    
    var hasStore: Bool {
        !(persistentContainer.persistentStoreCoordinator.persistentStores.isEmpty)
    }
    
    init(withConfiguration config: NewsApiDemoCoreDataStackConfiguration = NewsApiDemoCoreDataStackConfiguration()) {
    ...
    func deleteStore() -> NewsApiDemoVoidResult<Error> {
        ...
        guard let persistentStoreURL = storeURL else {
            return .failure(NewsApiDemoError.coreDataFailure(reason: .storeURLUnavailable))
        }
        do {
...
private extension NewsApiDemoCoreDataStack {
...
private extension NewsApiDemoCoreDataStack {
    ...
    func checkStore() -> NewsApiDemoError? {
        guard !hasStore else {
            return nil
        }
